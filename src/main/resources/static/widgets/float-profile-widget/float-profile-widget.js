...
`,X=[{key:`displayName`,label:`应用名称`,required:!0},{key:`miniProgramCode`,label:`太阳码（图片地址）`,required:!0},{key:`appId`,label:`小程序 AppID`,required:!0,placeholder:`小程序 AppID，如 wx1234567890abcdef`},{key:`path`,label:`跳转页面路径`,required:!1,placeholder:`如 pages/index/index`},{key:`link`,label:`应用地址`,required:!1,placeholder:`#小程序://xxx`},{key:`description`,label:`应用描述`,required:!1,type:`textarea`},{key:`applyRemark`,label:`申请说明`,required:!1,type:`textarea`}],Z=[{key:`avatar`,label:`作者头像（图片地址）`,required:!1},{key:`authorName`,label:`作者昵称`,required:!1},{key:`website`,label:`作者网站`,required:!1},{key:`email`,label:`邮箱（选填，用于审核结果通知）`,required:!1}],Q=`uh-fpw-apply-draft`;function $(e){return e?/^(https?:)?\/\//.test(e)||/^data:/i.test(e)?e:window.location.origin+(e.startsWith(`/`)?e:`/`+e):``}var ke=class extends W{static{this.styles=[Oe]}static{this.properties={applyOpen:{state:!0},linksOpen:{state:!0},applySubmitting:{state:!0},captchaId:{state:!0},captchaSrc:{state:!0},linksLoading:{state:!0},linksError:{state:!0},miniInfo:{state:!0},blogger:{state:!0},groupOptions:{state:!0},applyTab:{state:!0},screenshotRows:{state:!0},minimized:{state:!0},edgeTrigger:{state:!0},edgeSide:{state:!0},edgeTriggerStyle:{state:!0},miniDotStyle:{state:!0}}}constructor(){super(),this.dragState=null,this.miniDotDrag=null,this.miniDotDragged=!1,this.config=G,this.applyOpen=!1,this.linksOpen=!1,this.applySubmitting=!1,this.captchaId=``,this.captchaSrc=``,this.linksLoading=!1,this.linksError=!1,this.miniInfo=null,this.blogger=null,this.groupOptions=[],this.applyTab=`basic`,this.screenshotRows=[``],this.minimized=!1,this.edgeTrigger=!1,this.edgeSide=``,this.edgeTriggerStyle=``,this.miniDotStyle=``}connectedCallback(){super.connectedCallback(),(!De()||this.isClosed())&&this.remove()}firstUpdated(){this.applyPosition(),requestAnimationFrame(()=>this.applyDefaultState())}applyDefaultState(){let e=this.cardEl;if(!e)return;let t=xe();if(t){if(t.cardPos&&this.setFree(t.cardPos.left,t.cardPos.top),t.edge){this.hideToEdge(t.edge,!1);return}if(t.minimized){if(t.dotPos)this.miniDotStyle=`left:${Math.round(t.dotPos.left)}px;top:${Math.round(t.dotPos.top)}px;`;else{let t=e.getBoundingClientRect();this.miniDotStyle=`left:${Math.round(t.left)}px;top:${Math.round(t.top)}px;`}this.minimized=!0;return}return}if(this.config.defaultState===`minimized`){let t=e.getBoundingClientRect();this.miniDotStyle=`left:${Math.round(t.left)}px;top:${Math.round(t.top)}px;`,this.minimized=!0}}persistState(e){Se({...xe()??{minimized:!1,edge:null},...e})}isClosed(){if(this.config.rememberClosed===!1)return!1;try{return sessionStorage.getItem(K)===`1`}catch{return!1}}get cardEl(){return this.renderRoot.querySelector(`.uh-fpw`)}applyPosition(){let e=this.cardEl;if(!e)return;let t=this.config,n=t.position||`bottom-right`;e.classList.add(`uh-fpw-pos-`+n),t.dragEnabled&&e.classList.add(`uh-fpw-draggable`);let r=Number(t.offsetX)||0,i=Number(t.offsetY)||0,a=n===`center`||n===`top-center`||n===`bottom-center`?`calc(-50% + `+r+`px)`:r+`px`,o=n===`center`||n===`left-center`||n===`right-center`?`calc(-50% + `+i+`px)`:i+`px`;e.style.transform=`translate(`+a+`, `+o+`)`}onPointerDown(e){if(!this.config.dragEnabled)return;let t=e.target;if(t&&(t.closest(`.uh-fpw-close`)||t.closest(`.uh-fpw-minimize`)||t.closest(`.uh-fpw-actions`)||t.closest(`.uh-fpw-overlay`)||t.closest(`.uh-fpw-mini-dot`)||t.closest(`.uh-fpw-edge-trigger`)))return;let n=this.cardEl;if(!n)return;let r=n.getBoundingClientRect();this.dragState={startX:e.clientX,startY:e.clientY,left:r.left,top:r.top},this.restoreFromEdge(),n.classList.add(`uh-fpw-dragging`),n.style.touchAction=`none`,n.setPointerCapture(e.pointerId),e.preventDefault()}onPointerMove(e){let t=this.dragState,n=this.cardEl;if(!t||!n)return;let r=t.left+(e.clientX-t.startX),i=t.top+(e.clientY-t.startY);r=Math.max(0,Math.min(r,window.innerWidth-n.offsetWidth)),i=Math.max(0,Math.min(i,window.innerHeight-n.offsetHeight)),this.setFree(r,i)}onPointerEnd(){let e=this.cardEl;if(!this.dragState||!e)return;this.dragState=null,e.classList.remove(`uh-fpw-dragging`),e.style.touchAction=``;let t=e.getBoundingClientRect();this.maybeEdgeHide(),this.edgeTrigger||this.persistState({edge:null,minimized:!1,cardPos:{left:Math.round(t.left),top:Math.round(t.top)}})}setFree(e,t){let n=this.cardEl;n&&(n.style.left=e+`px`,n.style.top=t+`px`,n.style.right=`auto`,n.style.bottom=`auto`,n.style.transform=`translate(0, 0)`,n.classList.remove(`uh-fpw-edge`))}maybeEdgeHide(){if(!this.config.edgeHideEnabled)return;let e=this.cardEl;if(!e)return;let t=e.getBoundingClientRect(),n={left:t.left,right:window.innerWidth-t.right,top:t.top,bottom:window.innerHeight-t.bottom},r=null,i=this.config.edgeHideDistance,a=i==null?NaN:i===0?4:Number(i),o=Number.isFinite(a)?a:80;[`left`,`right`,`top`,`bottom`].forEach(e=>{n[e]<o&&(o=n[e],r=e)}),r&&this.hideToEdge(r,!0)}hideToEdge(e,t){let n=this.cardEl;if(!n)return;let r=n.getBoundingClientRect();e===`left`?n.style.transform=`translateX(${-(r.left+r.width)}px)`:e===`right`?n.style.transform=`translateX(${window.innerWidth-r.left}px)`:e===`top`?n.style.transform=`translateY(${-(r.top+r.height)}px)`:n.style.transform=`translateY(${window.innerHeight-r.top}px)`,n.classList.add(`uh-fpw-edge`,`uh-fpw-edge-`+e),this.edgeSide=e,this.edgeTriggerStyle=this.buildEdgeTriggerStyle(e,r),this.edgeTrigger=!0,t&&this.persistState({edge:e,minimized:!1})}buildEdgeTriggerStyle(e,t){let n=t.left+t.width/2,r=t.top+t.height/2;return e===`left`?`left:2px;top:${Math.round(r-17.5)}px;`:e===`right`?`right:2px;top:${Math.round(r-17.5)}px;`:e===`top`?`top:2px;left:${Math.round(n-17.5)}px;`:`bottom:2px;left:${Math.round(n-17.5)}px;`}onEdgeTriggerEnter(){let e=this.cardEl;e&&e.classList.add(`uh-fpw-edge-hover`)}onEdgeTriggerLeave(){let e=this.cardEl;e&&e.classList.remove(`uh-fpw-edge-hover`)}restoreFromEdge(){let e=this.cardEl;if(this.edgeTrigger=!1,this.edgeSide=``,this.edgeTriggerStyle=``,e){e.classList.contains(`uh-fpw-edge`)&&(e.style.transform=``),e.classList.remove(`uh-fpw-edge`,`uh-fpw-edge-hover`,`uh-fpw-edge-left`,`uh-fpw-edge-right`,`uh-fpw-edge-top`,`uh-fpw-edge-bottom`);let t=e.getBoundingClientRect();this.persistState({edge:null,minimized:!1,cardPos:{left:Math.round(t.left),top:Math.round(t.top)}})}}onMinimizeClick(){let e=this.cardEl;if(!e)return;let t=e.getBoundingClientRect();this.miniDotStyle=`left:${Math.round(t.left)}px;top:${Math.round(t.top)}px;`,this.minimized=!0,this.edgeTrigger=!1,e.classList.contains(`uh-fpw-edge`)&&(e.style.transform=``),e.classList.remove(`uh-fpw-edge`,`uh-fpw-edge-left`,`uh-fpw-edge-right`,`uh-fpw-edge-top`,`uh-fpw-edge-bottom`),this.persistState({minimized:!0,edge:null,dotPos:{left:Math.round(t.left),top:Math.round(t.top)},cardPos:{left:Math.round(t.left),top:Math.round(t.top)}})}onRestoreClick(){if(this.miniDotDragged){this.miniDotDragged=!1;return}let e=this.renderRoot.querySelector(`.uh-fpw-mini-dot`),t=this.cardEl;if(e&&t){let n=e.getBoundingClientRect();t.style.left=Math.round(n.left)+`px`,t.style.top=Math.round(n.top)+`px`,t.style.right=`auto`,t.style.bottom=`auto`,t.style.transform=``}this.minimized=!1,this.updateComplete.then(()=>{let e=this.cardEl;if(!e)return;let t=Number.parseInt(e.style.left,10)||0,n=Number.parseInt(e.style.top,10)||0,r=Math.max(0,window.innerWidth-e.offsetWidth),i=Math.max(0,window.innerHeight-e.offsetHeight);t>r&&(t=r,e.style.left=r+`px`),n>i&&(n=i,e.style.top=i+`px`),this.persistState({minimized:!1,edge:null,cardPos:{left:t,top:n}})})}onMiniDotPointerDown(e){let t=e.currentTarget,n=t.getBoundingClientRect();this.miniDotDrag={startX:e.clientX,startY:e.clientY,left:n.left,top:n.top,moved:!1},t.setPointerCapture(e.pointerId)}onMiniDotPointerMove(e){let t=this.miniDotDrag;if(!t)return;let n=e.clientX-t.startX,r=e.clientY-t.startY;if(!t.moved&&(Math.abs(n)>3||Math.abs(r)>3)&&(t.moved=!0),!t.moved)return;let i=e.currentTarget,a=Math.max(0,Math.min(t.left+n,window.innerWidth-i.offsetWidth)),o=Math.max(0,Math.min(t.top+r,window.innerHeight-i.offsetHeight));i.style.left=a+`px`,i.style.top=o+`px`}onMiniDotPointerEnd(){if(this.miniDotDrag?.moved){this.miniDotDragged=!0;let e=this.renderRoot.querySelector(`.uh-fpw-mini-dot`);e&&e.style.left&&e.style.top&&this.persistState({dotPos:{left:Number.parseFloat(e.style.left),top:Number.parseFloat(e.style.top)}})}this.miniDotDrag=null}onCloseClick(){if(this.config.rememberClosed!==!1)try{sessionStorage.setItem(K,`1`)}catch{}try{sessionStorage.removeItem(q)}catch{}let e=this.cardEl;e?(e.classList.add(`uh-fpw-closing`),setTimeout(()=>this.remove(),200)):this.remove()}onOverlayClick(e){e.target.classList.contains(`uh-fpw-overlay`)&&this.closeModals()}closeModals(){this.applyOpen=!1,this.linksOpen=!1}openApply(){this.applyOpen=!0,this.refreshCaptcha(),this.updateComplete.then(()=>this.restoreDraft())}restoreDraft(){let e=this.renderRoot.querySelector(`.uh-fpw-form`);if(!e)return;let t=this.loadDraft();[...X,...Z].forEach(n=>{let r=e.elements.namedItem(n.key);r&&t[n.key]&&(r.value=String(t[n.key]))});let n=t.screenshots;Array.isArray(n)?this.screenshotRows=n.length?n.slice():[``]:typeof n==`string`&&n.trim()&&(this.screenshotRows=n.split(`
`).map(e=>e.trim()).filter(Boolean),this.screenshotRows.length||(this.screenshotRows=[``]))}loadDraft(){try{let e=localStorage.getItem(Q);return e?JSON.parse(e):{}}catch{return{}}}saveDraft(){let e=this.renderRoot.querySelector(`.uh-fpw-form`);if(!e)return;let t={};[...X,...Z].forEach(n=>{let r=e.elements.namedItem(n.key);t[n.key]=r?.value||``}),t.screenshots=this.screenshotRows.filter(e=>e.trim());try{localStorage.setItem(Q,JSON.stringify(t))}catch{}}clearDraft(){try{localStorage.removeItem(Q)}catch{}}resetApply(){let e=this.renderRoot.querySelector(`.uh-fpw-form`);e&&e.reset(),this.screenshotRows=[``],this.applyTab=`basic`,this.clearDraft()}onFormInput(){this.saveDraft()}refreshCaptcha(){fetch(Ce).then(e=>e.json()).then(e=>{e&&e.imageBase64&&this.setCaptcha(e)}).catch(()=>{})}setCaptcha(e){this.captchaId=e.id,this.captchaSrc=e.imageBase64}async onApplySubmit(e){e.preventDefault();let t=e.target;for(let e of[{key:`displayName`,message:`请填写小程序名称`,panel:`basic`},{key:`miniProgramCode`,message:`请填写太阳码图片地址`,panel:`basic`},{key:`captchaCode`,message:`请输入验证码`}]){let n=t.elements.namedItem(e.key);if(!n||!n.value.trim()){e.panel&&(this.applyTab=e.panel),alert(e.message);return}}let n={};[...X,...Z].forEach(e=>{let r=t.elements.namedItem(e.key);r&&r.value.trim()&&(n[e.key]=r.value.trim())});let r=this.screenshotRows.map(e=>e.trim()).filter(Boolean).map($);r.length&&(n.screenshots=r);let i=(t.elements.namedItem(`captchaCode`)?.value||``).trim(),a=`?captchaId=`+encodeURIComponent(this.captchaId||``)+`&captchaCode=`+encodeURIComponent(i);this.applySubmitting=!0;try{let e=await fetch(we+a,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({spec:n})}),r=await e.json().catch(()=>({}));if(e.status===200||e.status===201){this.applyOpen=!1,this.clearDraft(),t.reset(),this.screenshotRows=[``],this.applyTab=`basic`,alert(`申请提交成功，请等待审核`);return}e.status===403&&r.captcha&&this.setCaptcha(r.captcha),alert(r.message||`提交失败，请重试`)}catch{alert(`网络异常，请稍后重试`)}finally{this.applySubmitting=!1}}openLinks(){this.linksOpen=!0,this.linksLoading=!0,this.linksError=!1,this.miniInfo=null,this.blogger=null,fetch(Te).then(e=>e.json()).then(e=>{let t=e?.featureConfig;this.miniInfo=t?.linkInfo?.miniInfo||null,this.blogger=t?.profile?.blogger||null}).catch(()=>{this.linksError=!0}).finally(()=>{this.linksLoading=!1})}render(){let e=this.config,t=Number(e.cardWidth)||100;return N`
      ...
      >
        <div class="uh-fpw-main" ?hidden=${this.minimized}>
          <div class="uh-fpw-topbar">
            ...
            ${e.closeEnabled===!1?``:N`<button type="button" class="uh-fpw-close" aria-label="关闭悬浮窗" @click=${this.onCloseClick}>&times;</button>`}
          </div>
          ${e.imageUrl?N`<img class="uh-fpw-img" src=${$(e.imageUrl)} alt=${e.name||`小程序太阳码`} />`:``}
          ${e.name?N`<div class="uh-fpw-name" style="font-size:${Number(e.nameSize)||14}px;color:${e.nameColor||`#333333`}">${e.name}</div>`:``}
          ${e.description?N`<div class="uh-fpw-desc" style="font-size:${Number(e.descSize)||12}px;color:${e.descColor||`#999999`}">${e.description}</div>`:``}
          ${e.miniProgramApply?N`
      ...
      ${this.minimized?N`
            <button
              ...
              aria-label="恢复悬浮窗"
            >
              ${e.imageUrl?N`<img src=${$(e.imageUrl)} alt="" />`:``}
      ...
      ${this.edgeTrigger?N`
            <button
              ...
              aria-label="展开悬浮窗"
            ></button>`:``}
    `}renderApplyField(e){let t=N`<span>${e.label}${e.required?` *`:``}</span>`;return e.type===`select`?N`
        <label class="uh-fpw-field">
          ${t}
          <select name=${e.key} class="uh-fpw-select">
            <option value="">未分组</option>
            ${this.groupOptions.map(e=>N`<option value=${e.value}>${e.label}</option>`)}
          </select>
        </label>`:e.type===`textarea`?N`
        <label class="uh-fpw-field">
          ${t}
          <textarea name=${e.key} rows="2" class="uh-fpw-textarea" placeholder=${e.placeholder??``}></textarea>
        </label>`:N`
      <label class="uh-fpw-field">
        ${t}
        <input type="text" name=${e.key} ?required=${e.required} placeholder=${e.placeholder??``} />
      </label>`}renderApplyModal(){return N`
    ...
    `}renderScreenshotRows(){return N`
      <div class="uh-fpw-field">
        <span>预览截图(可选)</span>
        ${this.screenshotRows.map((e,t)=>N`
      ...
      <div class="uh-fpw-overlay" @click=${this.onOverlayClick}>
        <div class="uh-fpw-modal">
          ...
          <div class="uh-fpw-modal-body">
            ${this.linksLoading?N`<div class="uh-fpw-loading">加载中…</div>`:this.linksError?N`<div class="uh-fpw-empty">加载失败，请稍后重试</div>`:n?N`
                      ...
                      >复制全部</button>
                    `:N`<div class="uh-fpw-empty">暂无友链信息</div>`}
          </div>
        </div>
      </div>
    `}renderCopyRow(e,t,n){if(!t)return``;let r=!!n?.textarea,i=n?.copyable??!0;return N`
      <div class="uh-fpw-copy-row">
        <span class="uh-fpw-copy-label">${e}</span>
        ${r?N`
          <textarea
...
`)}};customElements.define(`uh-float-profile-widget`,ke);function Ae(){if(document.querySelector(`uh-float-profile-widget`))return;let e=document.createElement(`uh-float-profile-widget`);document.body.appendChild(e)}G&&typeof G==`object`&&(document.body?Ae():document.addEventListener(`DOMContentLoaded`,Ae))})();