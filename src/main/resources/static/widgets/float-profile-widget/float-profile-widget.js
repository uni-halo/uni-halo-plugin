...
`).map(e=>e.trim()).filter(Boolean),this.screenshotRows.length||(this.screenshotRows=[``]))}loadDraft(){try{let e=localStorage.getItem(Q);return e?JSON.parse(e):{}}catch{return{}}}saveDraft(){let e=this.renderRoot.querySelector(`.uh-fpw-form`);if(!e)return;let t={};[...X,...Z].forEach(n=>{let r=e.elements.namedItem(n.key);t[n.key]=r?.value||``}),t.screenshots=this.screenshotRows.filter(e=>e.trim());try{localStorage.setItem(Q,JSON.stringify(t))}catch{}}clearDraft(){try{localStorage.removeItem(Q)}catch{}}resetApply(){let e=this.renderRoot.querySelector(`.uh-fpw-form`);e&&e.reset(),this.screenshotRows=[``],this.applyTab=`basic`,this.clearDraft()}onFormInput(){this.saveDraft()}refreshCaptcha(){fetch(be).then(e=>e.json()).then(e=>{e&&e.imageBase64&&this.setCaptcha(e)}).catch(()=>{})}setCaptcha(e){this.captchaId=e.id,this.captchaSrc=e.imageBase64}async onApplySubmit(e){e.preventDefault();let t=e.target;for(let e of[{key:`displayName`,message:`请填写小程序名称`,panel:`basic`},{key:`miniProgramCode`,message:`请填写太阳码图片地址`,panel:`basic`},{key:`captchaCode`,message:`请输入验证码`}]){let n=t.elements.namedItem(e.key);if(!n||!n.value.trim()){e.panel&&(this.applyTab=e.panel),alert(e.message);return}}let n={};[...X,...Z].forEach(e=>{let r=t.elements.namedItem(e.key);r&&r.value.trim()&&(n[e.key]=r.value.trim())});let r=this.screenshotRows.map(e=>e.trim()).filter(Boolean).map($);r.length&&(n.screenshots=r);let i=(t.elements.namedItem(`captchaCode`)?.value||``).trim(),a=`?captchaId=`+encodeURIComponent(this.captchaId||``)+`&captchaCode=`+encodeURIComponent(i);this.applySubmitting=!0;try{let e=await fetch(xe+a,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({spec:n})}),r=await e.json().catch(()=>({}));if(e.status===200||e.status===201){this.applyOpen=!1,this.clearDraft(),t.reset(),this.screenshotRows=[``],this.applyTab=`basic`,alert(`申请提交成功，请等待审核`);return}e.status===403&&r.captcha&&this.setCaptcha(r.captcha),alert(r.message||`提交失败，请重试`)}catch{alert(`网络异常，请稍后重试`)}finally{this.applySubmitting=!1}}openLinks(){this.linksOpen=!0,this.linksLoading=!0,this.linksError=!1,this.miniInfo=null,this.blogger=null,fetch(Se).then(e=>e.json()).then(e=>{let t=e?.featureConfig;this.miniInfo=t?.linkInfo?.miniInfo||null,this.blogger=t?.profile?.blogger||null}).catch(()=>{this.linksError=!0}).finally(()=>{this.linksLoading=!1})}render(){let e=this.config,t=Number(e.cardWidth)||100;return P`
      ...
      >
        <div class="uh-fpw-main" ?hidden=${this.minimized}>
          <div class="uh-fpw-topbar">
            ...
            ${e.closeEnabled===!1?``:P`<button type="button" class="uh-fpw-close" aria-label="关闭悬浮窗" @click=${this.onCloseClick}>&times;</button>`}
          </div>
          ${e.imageUrl?P`<img class="uh-fpw-img" src=${$(e.imageUrl)} alt=${e.name||`小程序太阳码`} />`:``}
          ${e.name?P`<div class="uh-fpw-name" style="font-size:${Number(e.nameSize)||14}px;color:${e.nameColor||`#333333`}">${e.name}</div>`:``}
          ${e.description?P`<div class="uh-fpw-desc" style="font-size:${Number(e.descSize)||12}px;color:${e.descColor||`#999999`}">${e.description}</div>`:``}
      ...
      ${this.minimized?P`
            <button
              ...
              aria-label="恢复悬浮窗"
            >
              ${e.imageUrl?P`<img src=${$(e.imageUrl)} alt="" />`:``}
      ...
      <div class="uh-fpw-overlay" @click=${this.onOverlayClick}>
        <div class="uh-fpw-modal">
          ...
          <div class="uh-fpw-modal-body">
            ${this.linksLoading?P`<div class="uh-fpw-loading">加载中…</div>`:this.linksError?P`<div class="uh-fpw-empty">加载失败，请稍后重试</div>`:n?P`
                      ...
                      >复制全部</button>
                    `:P`<div class="uh-fpw-empty">暂无友链信息</div>`}
          </div>
        </div>
      </div>
...
`)}};customElements.define(`uh-float-profile-widget`,Ee);function De(){if(document.querySelector(`uh-float-profile-widget`))return;let e=document.createElement(`uh-float-profile-widget`);document.body.appendChild(e)}K&&typeof K==`object`&&(document.body?De():document.addEventListener(`DOMContentLoaded`,De))})();